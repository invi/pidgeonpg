 ...
 */
  generate: function(skey, primarySki) {
    ...
    var IV, saltValue, ski, passphrase;
    if (primarySki) {
      ski = primarySki;
      IV = misc.atos(ski.iv);
      saltValue = misc.atos(ski.s2k.salt);
      passphrase = this.getCachedPassphrase(ski);
    } else { 
      IV =  misc.atos(prng.getService(Ci.nsIRandomGenerator).generateRandomBytes(NOB, "asdfqwersadf"));  
      saltValue = misc.atos(prng.getService(Ci.nsIRandomGenerator).generateRandomBytes(NOB, "asdfsfsdfsd"));  
      ski = { 
              usage: 254,
              s2k: {
                mode: 3,
      ...
      passphrase = this.enterNewPassphrase(ski);
    }
    ...
    return {skey: [wrapped_skey], ski: ski};
  },
